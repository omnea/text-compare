...
module.exports = class AddressComparator {
	...
	*/
	...
	prepareAddressFields(lang, address){
		address.street = address.street ? address.street = this.prepareStreetField(lang, address.street) : "";
	...
	prepareStreetField(lang, street) {
		if (!street) {
			return "";
		}

		var field = this.core.removeMultipleWhiteSpaces(street.toLowerCase());
		field = this.core.normalizeCharacters(lang.getName(), field);
		field = this.core.trimSpaces(field);
